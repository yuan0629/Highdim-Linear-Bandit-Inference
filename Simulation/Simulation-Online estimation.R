...
gamma <- 1/3 
c2 <- 1 # define the exploration probability \varepsilon_t=c_2t^{-gamma}; change between "gamma <- 1/3" and "gamma <- 1/2" to 
...
if (gamma == 1/3) {
  save(result1,
       file = here::here("Simulation", "Simulation-result",
                         paste0("scenario", scenario, "-sgd-result1.RData")))
} else if (gamma == 1/2) {
  result2 <- result1
  save(result2,
       file = here::here("Simulation", "Simulation-result",
                         paste0("scenario", scenario, "-sgd-result2.RData")))
}
...
save(result3,
     file = here::here("Simulation", "Simulation-result",
                       paste0("scenario", scenario, "-sgd-result3.RData")))
